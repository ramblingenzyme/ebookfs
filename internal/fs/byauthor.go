...
func newByAuthorDir(f *fs.FS, reg *bookRegistry, books []*model.Book) *byAuthorDir {
	d := &byAuthorDir{
		StaticDir: *fs.NewStaticDir(f.NewStat("by-author", "glenda", "glenda", 0555|proto.DMDIR)),
		f:         f,
		reg:       reg,
	}
...
func (d *byAuthorDir) add(book *model.Book) {
	// A book with multiple authors appears under each of them.
