...
// cleanupLoop sweeps expired handles until Close. Nothing synchronises its
// startup, so a short test may exit before it runs: coverage swings between 0%
// and 62.5%, and any gate over this package has to serialize (-p 1). The ticker
// arm never runs under test, since five minutes outlives every run.
