...
func TestSeriesEntryName_PadTriggeredByMaxIndex(t *testing.T) {
	...
	b1 := makeBook(1, "Tenth", "Author")
	b1.Series = &model.SeriesRef{Name: "S", Index: 10.0}
	b2 := makeBook(2, "Eleventh", "Author")
	b2.Series = &model.SeriesRef{Name: "S", Index: 11.0}
	...
	// With maxIdx >= 10, entries should be zero-padded
	for _, name := range children {
		if len(name) < 3 {
			t.Errorf("entry %q too short for zero-padded format", name)
		}
	}
}
