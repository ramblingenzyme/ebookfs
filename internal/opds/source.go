...
// pageSize is how many publications one acquisition feed page carries. OPDS
// clients follow rel="next", so this only bounds one response.
const pageSize = 50

// recentLimit caps the recently-added feed. It is a single page by design: a
// "what's new" feed that paginates to the whole library is just the all-books
// feed in a different order.
...
// the books carrying it. A value containing a colon is safe because only the
// first is a separator; one containing a slash survives because the href
// escapes it and the handler hands back the decoded path.
type catalog struct {
	lib  Library
	rend Renderer
	// base is the configured absolute base URL, or "". The OpenSearch
	// description is the one document whose URL must be absolute (OpenSearch
	// 1.1 §4.4, "Url/@template"), and the handler only absolutizes a template
	// it built itself. With no base configured a relative template is all
	// there is, and clients resolve it against the document's own URL.
	base string
}
...
//
// ponytail: terms never reach the author field, because Query ANDs across
// fields and so cannot express "title OR author" in one call. Clients wanting
// an author use the advertised {author} parameter. The upgrade is an
// index-level OR, not a union assembled here.
...
// books runs q and renders the result as one page of an acquisition feed.
...
// page fills f with the books belonging to the requested page and adds the
// pagination links.
//
// ponytail: the caller has already loaded every matching book and this slices
// one page out of them, so page 20 costs the same as page 1. Fine for a
// personal library; the upgrade is an Offset on index.Query, at which point
// the slice here becomes the query's own LIMIT/OFFSET.
...
// facets renders one of the library's facet listings as a navigation feed,
// keeping the order the index returned. The count rides along as the entry's
// description, which is the only place OPDS 1.2 navigation has for it.
...
// statuses lists the reading-status vocabulary. Unlike the other three
// listings it is a fixed set, not a query: a status with no books is still a
// status, and the empty feed says so.
...
// publication renders a book as a catalog entry. Rating and reading status
// have no OPDS slot and are left out: the status feeds above already navigate
// by status, and nothing consumes a rating.
func (c *catalog) publication(b *library.Book) opds.Publication {
	...
	for scheme, value := range b.Identifiers() {
		p.Identifier(identifierURN(scheme, value))
	}
	for _, t := range b.Tags() {
		p.About(t)
	}
	if s := b.Series(); s != nil {
		// OPDS 2.0 only, and its position is a float, which EPUB 3.3 D.3.7
		// indexes are not: "98.4" there means volume 98, issue 4. A
		// multi-level index is left at zero rather than rendered as a number
...
// book resolves a decimal book id from a URL path.
func (c *catalog) book(id string) (*library.Book, error) {
	...
	b, err := c.lib.Get(n)
	if err != nil {
		// Any other failure is the index's, and the handler's default error
		// mapping turns it into a 500.
		if isNotFound(err) {
			return nil, opds.ErrNotFound
		}
		return nil, err
...
// published parses the date the epub carries. EPUB 3.3 recommends a complete
// date, but real files hold a year or a date alone, so all three shapes are
// accepted and anything else is dropped rather than guessed at.
