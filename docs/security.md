...
- The OPDS catalog is off by default (`opds.listen` is empty, so no second port opens). When it is switched on it is unauthenticated and plain HTTP, the same as the 9P listener, and the OPDS protocol itself has no write operations.
...
An enabled OPDS port is narrower and still serious: anyone who can reach it can browse the catalog and download every book in the library, including its covers and metadata. It cannot edit, delete or ingest anything.
...
- **Set `opds.base_url`** whenever the catalog is reachable from outside the host. Without it, the absolute URLs in served documents are built from the client's own `Host` and `X-Forwarded-*` headers, which a caller controls — a poisoning vector where a shared cache sits in front.
