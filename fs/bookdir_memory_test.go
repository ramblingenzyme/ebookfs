...
func TestBookDirMemoryFootprint(t *testing.T) {
	t.Logf("unsafe.Sizeof breakdown (fixed struct size, excludes heap-pointed data):")
	t.Logf("  proto.Qid       = %d B", unsafe.Sizeof(proto.Qid{}))
	t.Logf("  proto.Stat      = %d B", unsafe.Sizeof(proto.Stat{}))
	t.Logf("  fs.StaticDir    = %d B", unsafe.Sizeof(fs.StaticDir{}))
	t.Logf("  fs.BaseFile     = %d B", unsafe.Sizeof(fs.BaseFile{}))
	t.Logf("  fs.StaticFile   = %d B", unsafe.Sizeof(fs.StaticFile{}))
	t.Logf("  fieldFile       = %d B", unsafe.Sizeof(fieldFile{}))
	t.Logf("  epubFile        = %d B", unsafe.Sizeof(epubFile{}))
	t.Logf("  coverFile       = %d B", unsafe.Sizeof(coverFile{}))
	t.Logf("  model.Book      = %d B", unsafe.Sizeof(model.Book{}))
	t.Logf("  bookDir         = %d B", unsafe.Sizeof(bookDir{}))
	t.Logf("")
	...
	for _, n := range []int{100, 1000, 10000} {
		for _, withCover := range []bool{false, true} {
			...
			t.Logf("N=%-6d cover=%-5v  per-book=%5.0f B  per-bookDir(marginal)=%5.0f B  total/book=%5.0f B",
				n, withCover, perBook, perDir, perTotal)
		}
	}
	t.Logf("")
	t.Logf("Projections (per-book + per-bookDir), linear extrapolation from N=10000 with cover:")
	perBook := measureBooksOnly(10000, true)
	perTotal := measureBooksWithDirs(10000, true)
	perDir := perTotal - perBook
	t.Logf("  baseline: per-book=%.0f B  per-bookDir(marginal)=%.0f B  total/book=%.0f B",
		perBook, perDir, perTotal)
	for _, k := range []int{1000, 5000, 10000, 50000, 100000} {
		t.Logf("  %6d books  →  %7.1f MiB  (%5.0f B/book × %d)",
