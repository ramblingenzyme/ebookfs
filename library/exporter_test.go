...
func TestEpubExporter_Size_ReportsRecordedSize(t *testing.T) {
	...
	exp := epubExporter{}
	size, ok := exp.Size(testutil.WrapBook(book))
	if !ok {
...
func TestEpubExporter_Filename(t *testing.T) {
	...
	name := exp.Filename(testutil.WrapBook(book))
	if name != "mybook.epub" {
		...
		t.Run(kind, func(t *testing.T) {
			for _, tt := range tests {
				t.Run(tt.name, func(t *testing.T) {
					...
					if got := newExp(tt.statuses).Includes(testutil.WrapBook(book)); got != tt.want {
...
func TestKepubCacheDelegates(t *testing.T) {
	...
	if fn := kc.Filename(testutil.WrapBook(b)); fn != "mybook.kepub.epub" {
		t.Errorf("Filename = %q, want %q", fn, "mybook.kepub.epub")
	}

	if dn := kc.Dirname(testutil.WrapBook(b)); dn != "Alice" {
		t.Errorf("Dirname = %q, want %q", dn, "Alice")
	}

	_, ok := kc.Size(testutil.WrapBook(b))
	if ok {
		t.Error("Size should report cold for missing cache file")
	}

	// Warm must not panic.
	kc.Warm(testutil.WrapBook(b))

	// Open returns an error because the dummy source returns nil.
	_, err := kc.Open(testutil.WrapBook(b))
	if err == nil {
...
func TestEpubExporter_Dirname(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			b.Authors = authors
			got := exp.Dirname(testutil.WrapBook(b))
			if got != tt.want {
