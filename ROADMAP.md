...
| `search/` directory | Plan 9 clone-style API rather than path-encoded queries. Opening `search/clone` allocates a new search and reading it back returns an id; walking to `search/<id>/` gives a `ctl` file that a query is written to (same prefixes as before: `title:`, `author:`, `tag:`, `series:`, `status:`, `id:`, compound with `+`, e.g. `tag:sci-fi+status:unread`). The write executes the search and populates the directory with the matching books, which stay live (edits/deletes keep reflecting) rather than being a frozen snapshot. Closing the handle is an explicit step rather than being triggered by clunking `ctl`, since a client that writes its query and disconnects in the same breath (a plain shell redirect, for instance) shouldn't lose its results before it gets to browse them — that also means handles that are never explicitly closed need some backstop cleanup, still to be worked out. Rejected a flat path-encoded design (`search/title:foundation/`) and a faceted walk (`search/author:/<value>/tags:/<value>`): both require synthesizing a directory tree keyed by an unbounded query space on every lookup, with no natural point to garbage-collect it, unlike the fixed by-tag/by-status key sets. The rest of the `search/<id>/` interface — the `ctl` write/read format, refining vs. replacing a query, exact close mechanics — is still open. | Extended query methods (V2 #6) provide the Library-level `Search` API this wraps. |
