...
| Drift detection: single source for the epub's size | The index records the epub's byte size twice — once as the parser saw it, once as the drift check saw it — because the parser silently falls back to zero when its own stat fails, leaving a value the drift check can't trust. Nothing but a schema comment distinguishes them, and the untrustworthy copy is the one reported as the file's length over 9P and used for export sizing. Fold them into a single always-observed value, so the zero case stops being representable. | Drift detection |
| Drift detection: observing file state belongs to the store | The store owns on-disk layout, yet the library layer stats book files itself, composing their paths by hand — the one place it reaches past the store to the filesystem. It does so because the type describing observed file state lives in the index, which the store cannot import (they are siblings). Moving that type to the shared model package lets the store expose the observation as an operation and lets the sidecar's filename go back to being private to the store. Until then, renaming or reshaping the sidecar is a store-local change that has to be chased into the library by hand, with no compile error when it's missed. | Drift detection |
